...
import mageflow
from mageflow.signature.creator import resolve_signature_keys
...
async def test__resolve_signature_keys__all_task_signatures__returns_as_is():
    # Arrange
    sigs = [
        await mageflow.sign(f"task_{i}", model_validators=ContextMessage)
        for i in range(3)
    ]

    # Act
    result = await resolve_signature_keys(sigs)
...
async def test__resolve_signature_keys__all_string_keys__batch_fetches():
    ...
    # Act
    result = await resolve_signature_keys(keys)
...
async def test__resolve_signature_keys__all_hatchet_tasks__batch_creates(hatchet_tasks):
    # Act
    result = await resolve_signature_keys(hatchet_tasks)
...
async def test__resolve_signature_keys__mixed_types__preserves_order(hatchet_tasks):
    # Arrange
    existing_sig = await mageflow.sign("existing_task", model_validators=ContextMessage)
    pre_saved = await mageflow.sign("pre_saved_task", model_validators=ContextMessage)
    hatchet_task = hatchet_tasks[0]

    inputs = [existing_sig, pre_saved.key, hatchet_task]

    # Act
    result = await resolve_signature_keys(inputs)
...
async def test__resolve_signature_keys__missing_string_keys__returns_none():
    # Arrange
    existing = await mageflow.sign("existing_task", model_validators=ContextMessage)
...
async def test__resolve_signature_keys__empty_list__returns_empty():
    # Act
    result = await resolve_signature_keys([])
