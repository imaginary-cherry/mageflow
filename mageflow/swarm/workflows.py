...
from mageflow.signature.model import TaskSignature, _create_empty_model
...
async def swarm_start_tasks(msg, ctx):
    try:
        invoker = _create_invoker(msg, ctx)
        await invoker.log(f"Swarm task started {msg}")
        ...
        if swarm_task.has_swarm_started:
            await invoker.log(f"Swarm task started but already running {msg}")
            return
        ...
        await invoker.log(f"Swarm task started running {swarm_task.config.max_concurrency} tasks")
    except Exception:
        await invoker.log(f"MAJOR - Error in swarm start tasks")
        raise


async def swarm_item_done(msg: SwarmResultsMessage, ctx):
    invoker = _create_invoker(msg, ctx)
    ...
    try:
        ...
        await invoker.log(f"Swarm item done {swarm_item_id}")

        # Update swarm tasks
        swarm_task = await SwarmTaskSignature.aget(swarm_task_id)

        await invoker.log(f"Swarm item done {swarm_item_id} - saving results")
        ...
        await invoker.log(f"MAJOR - Error in swarm start item done")
        raise
    finally:
        await TaskSignature.remove_from_key(task_key)


async def swarm_item_failed(msg, ctx):
    invoker = _create_invoker(msg, ctx)
    task_data = invoker.task_ctx
    task_key = task_data[TASK_ID_PARAM_NAME]
    try:
        ...
        await invoker.log(f"Swarm item failed {swarm_item_key}")
        ...
        await invoker.log(f"MAJOR - Error in swarm item failed")
        raise
    finally:
        await TaskSignature.remove_from_key(task_key)


async def fill_swarm_running_tasks(msg: SwarmMessage, ctx):
    invoker = _create_invoker(msg, ctx)
    async with SwarmTaskSignature.alock_from_key(
        msg.swarm_task_id, action=SWARM_ACTION_FILL
    ) as swarm_task:
        if swarm_task.has_swarm_failed():
            await invoker.log(f"Swarm failed too much {msg.swarm_task_id}")
            ...
            await swarm_task.activate_error(empty_msg)
        ...
        if num_task_started:
            await invoker.log(f"Swarm item started new task {num_task_started}/{swarm_task.key}")
        else:
            await invoker.log(f"Swarm item no new task to run in {swarm_task.key}")
        ...
        if is_swarm_finished_running and not_yet_published:
            await invoker.log(f"Swarm item done - closing swarm {swarm_task.key}")
            await swarm_task.activate_success(msg)
            await swarm_task.done()
            await invoker.log(f"Swarm item done - closed swarm {swarm_task.key}")
