...
import { CheckCircle2, Circle, Loader2, XCircle, PauseCircle, Clock } from 'lucide-react';
...
const SimpleTaskNode = memo(({ data }: NodeProps) => {
  ...
  return (
    <div
      className={cn(
        ...
        'hover:shadow-xl hover:scale-105 min-w-[160px]',
        'bg-task-simple border-task-simple-border'
      )}
      onClick={() => onTaskClick?.(task)}
    >
      <Handle type="target" position={Position.Top} className="!bg-task-simple-border !w-3 !h-3" />
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-task-simple-border !w-3 !h-3" />
    </div>
  );
});

SimpleTaskNode.displayName = 'SimpleTaskNode';

export default SimpleTaskNode;
