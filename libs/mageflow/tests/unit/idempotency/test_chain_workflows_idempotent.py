...
async def test_chain_end_run_twice_callback_activated_once_idempotent(
    ...
    # Act
    await chain_end_task(setup.msg.chain_results, setup.lifecycle_manager, setup.logger)
    lifecycle2 = await adapter_with_lifecycle.lifecycle_from_signature(setup.msg, MagicMock(), setup.chain_signature.key)
...
async def test_chain_error_run_twice_callback_activated_once_idempotent(
    ...
    # Act
    await chain_error_task(setup.error_msg.chain_task_id, setup.error_msg.original_msg, setup.error_msg.error, setup.lifecycle_manager, setup.logger)
    lifecycle2 = await adapter_with_lifecycle.lifecycle_from_signature(setup.error_msg, MagicMock(), setup.chain_signature.key)
    await chain_error_task(setup.error_msg.chain_task_id, setup.error_msg.original_msg, setup.error_msg.error, lifecycle2, setup.logger)
