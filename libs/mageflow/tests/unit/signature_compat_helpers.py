...
def has_typevar(annotation) -> bool:
    if isinstance(annotation, typing.TypeVar):
        return True
    return any(has_typevar(arg) for arg in typing.get_args(annotation))


SKIP_KINDS = {inspect.Parameter.VAR_KEYWORD, inspect.Parameter.VAR_POSITIONAL}


def assert_child_accepts_all_parent_params(
    parent_cls: type,
    child_cls: type,
    method_name: str,
):
    parent_params = get_method_parameters(parent_cls, method_name)
    parent_hints = typing.get_type_hints(getattr(parent_cls, method_name))
    child_params = get_method_parameters(child_cls, method_name)
    ...
    for parent_param_name, parent_param in parent_params.items():
        ...
        assert is_explicitly_defined or is_in_unpack, (
            f"Parameter '{parent_param_name}' from {parent_name}.{method_name}() "
            f"is not accepted by {child_name}.{method_name}()"
        )

        if is_explicitly_defined:
            child_param = child_params[parent_param_name]
            assert child_param.default == parent_param.default, (
                f"Parameter '{parent_param_name}' in {child_name}.{method_name}() "
            ...
            if parent_hint is not None and not has_typevar(parent_hint):
                assert unpack_hint == parent_hint, (
                    f"Parameter '{parent_param_name}' in Unpack TypedDict for "
                    f"{child_name}.{method_name}() has type {unpack_hint}, "
