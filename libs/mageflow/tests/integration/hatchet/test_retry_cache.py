...
async def test__retry_cache__durable_task_retry__no_duplicate_signatures(
    ...
    assert (
        attempt_1_keys["sig1"] == attempt_2_keys["sig1"]
    ), f"sig1 mismatch: {attempt_1_keys['sig1']} != {attempt_2_keys['sig1']}"
    assert (
        attempt_1_keys["sig2"] == attempt_2_keys["sig2"]
    ), f"sig2 mismatch: {attempt_1_keys['sig2']} != {attempt_2_keys['sig2']}"
    assert (
        attempt_1_keys["chain_key"] == attempt_2_keys["chain_key"]
    ), f"chain_key mismatch: {attempt_1_keys['chain_key']} != {attempt_2_keys['chain_key']}"
    assert (
        attempt_1_keys["chain_sub1"] == attempt_2_keys["chain_sub1"]
    ), f"chain_sub1 mismatch"
    assert (
        attempt_1_keys["chain_sub2"] == attempt_2_keys["chain_sub2"]
    ), f"chain_sub2 mismatch"
    assert (
        attempt_1_keys["swarm_key"] == attempt_2_keys["swarm_key"]
    ), f"swarm_key mismatch: {attempt_1_keys['swarm_key']} != {attempt_2_keys['swarm_key']}"
    assert (
        attempt_1_keys["swarm_sub1"] == attempt_2_keys["swarm_sub1"]
    ), f"swarm_sub1 mismatch"
    assert (
        attempt_1_keys["swarm_sub2"] == attempt_2_keys["swarm_sub2"]
