...
class RedisModel(BaseModel):
    ...
    def _deserialize_list_for_load_fields(self, field_name: str, value: Any) -> Any:
        ...
        if list_args:
            ...
            for item in value:
                ...
                if isinstance(item, bytes) and actual_item_type != bytes:
                    item = item.decode()
    ...
    def _update_field_in_redis(
        ...
        if redis_type == "list":
            ...
            pipe.set(key, serialized_value, ex=TTL_SECONDS, xx=xx)
        elif redis_type == "string":
            pipe.set(key, serialized_value, ex=TTL_SECONDS, xx=xx)
        elif redis_type == "bytes":
            pipe.set(key, serialized_value, ex=TTL_SECONDS, xx=xx)
    ...
    async def append_to_list_from_key(cls, key: str, list_name: str, value: Any):
        ...
        async with redis_client.pipeline(transaction=True) as pipe:
            pipe.lpush(field_key, value)
            pipe.expire(field_key, TTL_SECONDS)
            await pipe.execute()
